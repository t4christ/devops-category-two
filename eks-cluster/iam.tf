...
data "aws_iam_policy_document" "eks-cluster-role-policy-json" {
  statement {
    ...
    principals {
      identifiers = ["eks.amazonaws.com", "autoscaling.amazonaws.com"]
      type        = "Service"
    }
  }
}
...
resource "aws_iam_role" "eks-cluster-role" {
  ...
  tags = merge(
    tomap({ "Name" = "${local.env}-${local.project}-eks-cluster-role" }),
  local.common_tags)
}
...
resource "aws_iam_role" "eks-worker-role" {
  ...
  tags = merge(
    tomap({ "Name" = "${local.env}-${local.project}-eks-worker-role" }),
  local.common_tags)
}
...
resource "aws_iam_role_policy_attachment" "eks-worker-role-AmazonEBSCSIDriverPolicy" {
